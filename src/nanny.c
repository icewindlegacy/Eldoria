...
/***************************************************************************************************/
		case CON_GET_OLD_PASSWORD:
			...
			{	write_to_buffer( d, "Wrong password.\n\r", 0 );
				...
				{	send_to_char(   "{M>{r>{C> {wBAD {WPASSWORD {wATTEMPT {C<{r<{M<{x\n\r{x",ch);
			...
			if ( IS_IMMORTAL(ch) )
			{	
                            printf_to_char(ch, "{CPlease Pick An Option{x\n\r\n\r{M{CE{r:{W Enter Eldoria\n\r{CL{r: {WLeave Eldoria\n\r{CS{r:{W Eldoria Wizlist\n\r{CW{r:{W Activate Wizinviz\n\r{CH{r:{W Who's on Eldoria\n\r{CD{r:{W Delete\n\r\n\r{CEnter Selection{W ->{x"); 
...
/***************************************************************************************************/    
        case CON_MENU:
			...
			{	case 'e': case 'E':
		        ...
		        case 'h': case 'H':
			       write_to_buffer( d, "Who's on Eldoria.\n\r", 0 );
			...
			case 'w': case 'W':
				...
				do_function(ch, &do_delete, ""); 
		                printf_to_char( ch, "Entering or leaving Eldoria will remove your delete status.\n\r" );
			        write_to_buffer( d, "\n\r*[HIT RETURN]*\n\r",0);                 
		                break;        
		...
		case CON_CREATION_MENU:
			...
			{	
				...
				*/
					...
					{	printf_to_char(ch, "\t{CNeutral Race%s{r:\n\r\t\t", race_count(ALIGN_NEUTRAL) == 1 ? "{r" : "s{r" );
						...
						{	if(counter==MAX_PC_RACE)
							...
							{	if (!race_table[race].pc_race)
									break;
								printf_to_char(ch, "{C%d{r:{W %s{x", counter, race_table[race].name,0);
					...
					{	printf_to_char(ch, "\t{CBenevolant Race%s{r:\n\r\t\t", race_count(ALIGN_NEUTRAL) == 1 ? "{r" : "s{r" );
					...
					{	printf_to_char(ch, "\t{CMalevolant Race%s{r:\n\r\t\t", race_count(ALIGN_NEUTRAL) == 1 ? "{r" : "s{r" );
				...
				case 'B':
					counter = 1;
					wrap = 0;
					send_to_char( "\n\r{CNow, a {Wclass{C must be chosen. These are your choices{r:{x\n\r",ch);
					for ( iClass = 0; iClass < MAX_CLASS; iClass++ )
					{	sprintf(buf, " {C%d{r:{W ", counter);
						send_to_char(buf,ch);
				...
				case 'D':
					...
					{	
		                            send_to_char("{CYou have already selected a home town!\n\rEnter selection{W->{x ",ch);
					    break;
					}
					send_to_char( "\n\r{CNow to select a Hometown. Your choices are{r:{x\n\r", ch );
					counter = 1;
					for (i=0;hometown_table[i].name != NULL; ++i)
					{	sprintf(buf,"{C%d{r:{W %s\n\r{x", counter, hometown_table[i].name );
						send_to_char(buf,ch);
						counter++;
					}
					send_to_char( "\n\r{CEnter hometown{W->{x ", ch);
					ch->pcdata->creation[CHOSE_HOMETOWN] = TRUE;
					d->connected = CON_GET_HOMETOWN;
					break;
				case 'E':
					counter = 1;
				    send_to_char("\n\r{CThe following Religions are available{r:\n\r", ch);
				...
				case 'F':
					...
					send_to_char("{C1{r:{W Please refrain from using actions within your short description.\n\r",ch);
					send_to_char("{C2{r:{W Please only put the most significant features of your character.\n\r",ch);
					send_to_char("{C3{r:{W Do not put common charactistics of your race in the short description.\n\r",ch);
					send_to_char("{C4{r:{W Make sure the description is valid in all positions, sitting, standing, ect...\n\r{x",ch);
...
/***************************************************************************************************/
	    ...
	    case CON_GET_HOMETOWN:
			...
			if (iClass <= -1 || !found)
			{	send_to_char( "\n\r{CThat's not a valid selection.\n\r", ch);
...
/***************************************************************************************************/
	    ...
	    case CON_GET_GOD:
			...
			{	send_to_char("{CYou have chosen the path of a Heathen. May the gods {rNOT{C be with you.{x\n\r\n\r", ch);
			...
			printf_to_char(ch, "{CYour faith is placed in {W%s{x.\n\r\n\r", pRlg->name );
...
/***************************************************************************************************/
		...
		case CON_SET_SHORT:
			...
			printf_to_char(ch, "{CYour short desc has been set too{r:{W %s{x\n\r",argument);
...
/***************************************************************************************************/
	    ...
	    case CON_GET_HAIR:
			...
			{	send_to_char("\n\r{CThat is not a valid colour. Your choices are{r:{x\n\r\n\r",ch);
				counter = 1;
				for(i = 0 ; i < MAX_HAIR ; i++)
				{	printf_to_char(ch, "{C%d{r:{W %s{x\n\r",counter, hair_table[i].name);
					counter++;
				}
				send_to_char("\n\r{CEnter hair colour{W->{x ",ch);
				break;
			}

			printf_to_char(ch, "{CYour hair colour is now {W%s{C.\n\r", hair_table[iClass].name);
			ch->pcdata->hair = iClass;
			d->connected = CON_GET_EYE;
			send_to_char("{CWhich colour eyes would you like?{x\n\r\n\r",ch);
			counter = 1;
			for(i = 0 ; i < MAX_EYE ; i++)
			{	printf_to_char(ch, "{C%d{r:{W %s{x\n\r",counter, eye_table[i].name);
				counter++;
			}
			send_to_char("\n\r{CEnter eye colour{W->{x ",ch);
			break;
...
/***************************************************************************************************/
		...
		case CON_GET_EYE:
			...
			printf_to_char(ch, "{CYour eye colour is now {W%s{C.{x\n\r", eye_table[iClass].name);
...
/***************************************************************************************************/
		...
		case CON_GET_HEIGHT:
			...
			{	send_to_char("\n\r{CThat is not a valid height. Your choices are{r:{x\n\r\n\r",ch);
				counter = 1;
				for(i = 0 ; i < MAX_HEIGHT ; i++)
				{	printf_to_char(ch, "{C%d{r:{W %s{x\n\r",counter, height_table[i].name);
					counter++;
			...
			printf_to_char(ch, "{CYour height is now {W%s{C.{x\n\r", height_table[iClass].name);
			ch->pcdata->height = iClass;
			d->connected = CON_GET_WEIGHT;
			send_to_char("{CWhat type of physique would you like?{x\n\r\n\r",ch);
...
/***************************************************************************************************/
	    ...
	    case CON_GET_WEIGHT:
			...
			{	send_to_char("\n\r{CThat is not a valid weight. Your choices are{r:{x\n\r\n\r",ch);
				counter = 1;
				for(i = 0 ; i < MAX_WEIGHT ; i++)
				{	printf_to_char(ch, "{C%d{r:{W %s{x\n\r",counter, weight_table[i].name);
					counter++;
...
/***************************************************************************************************/
	    ...
	    case CON_PICK_WEAPON:
		    ...
		    {	write_to_buffer ( d, "Sucessful Remort!\n\r", 0 );
		        ...
		        /* Clear any pending string pager */
		        if (d->showstr_head)
		        {
...
/***************************************************************************************************/
	    ...
	    case CON_READ_MOTD:
			...
			write_to_buffer( d, "\n\rWelcome to Eldoria Please do not feed the mobiles.\n\r", 0 );
			ch->next	= char_list;
			char_list	= ch;
			d->connected	= CON_PLAYING;
			/* Clear any pending string pager from help command during creation */
			if (d->showstr_head)
			{
				free_mem(d->showstr_head, strlen(d->showstr_head));
				d->showstr_head = NULL;
		...
		wiznet("{B[LOGIN]{x: {M$N has left real life behind.{x",ch,NULL, WIZ_LOGINS,WIZ_SITES,get_trust(ch));
                count_connections++;
          ...
          if (ch->desc->out_compress)
	        ...
	        {      
		         ...
		         act("{G[INFO]:{x {M$n has decided to leave real life behind.{x",ch,0,vch,TO_VICT);
                }
