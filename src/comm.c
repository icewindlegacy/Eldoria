...
const char mxp_will             [] = { (char)IAC, (char)WILL, (char)TELOPT_MXP,      '\0' };  // Challenge
const char mxp_do               [] = { (char)IAC, (char)DO,   (char)TELOPT_MXP,      '\0' };  // Granted
const char mxp_dont             [] = { (char)IAC, (char)DONT, (char)TELOPT_MXP,      '\0' };  // Denied

const char msp_will             [] = { (char)IAC, (char)WILL, (char)TELOPT_MSP,      '\0' };  // Challenge
const char msp_do               [] = { (char)IAC, (char)DO,   (char)TELOPT_MSP,      '\0' };  // Granted
const char msp_dont             [] = { (char)IAC, (char)DONT, (char)TELOPT_MSP,      '\0' };  // Denied

/* MCCP */
const char compress_will        [] = { (char)IAC, (char)WILL, (char)TELOPT_COMPRESS2, '\0' };
const char compress_do          [] = { (char)IAC, (char)DO,   (char)TELOPT_COMPRESS2, '\0' };
const char compress_dont        [] = { (char)IAC, (char)DONT, (char)TELOPT_COMPRESS2, '\0' };
	...
	for ( d = descriptor_list; d != NULL; d = d_next )
	{
		...
		stop_idling( d->character );
	        ...
	        if ( d->showstr_point )
	        {
	            ...
	            /* If pager was cleared by show_string, continue to process command */
	            if ( !d->showstr_point )
	            {
	                if ( d->pString )
      ...
      if( txt && ch->desc )
       {
           if( IS_SET( ch->act, PLR_COLOUR ) )
           {
               ...
               *point2 = '\0';    
               /* Only set up pager if content is long enough to need pagination */
               if ( strlen(buf) > 1000 ) {
...
void show_string(struct descriptor_data *d, char *input)
{
    ...
    /* If input is just whitespace (like hitting return), advance the pager */
	...
	if (!*d->showstr_point || (show_lines > 0 && lines >= show_lines))
	{
	    *scan = '\0';
...
void sig_handler(int sig)
{
  ...
  {
    case SIGINT:
      bug("Sig handler SIGINT (Ctrl+C).",0);
      do_auto_shutdown();
      break;
    case SIGBUS:
