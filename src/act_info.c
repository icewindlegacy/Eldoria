 ...
 *  Michael Seifert, Hans Henrik St{Mfeldt, Tom Madsen, and Katja Nyboe.   *
 *                                                                         *
...
{
 "{M<{Cused as light{M>{W            ",
 "{M<{Cworn as hood{M>{W            ",
 "{M<{Cworn on head{M>{W            ",
 "{M<{Cworn as cloak{M>{W           ",
 "{M<{Cworn around neck{M>{W        ",
 "{M<{Cworn around neck{M>{W        ",
 "{M<{Cworn on torso{M>{W           ",
 "{M<{Cworn on back{M>{W            ",
 "{M<{Cworn on shoulder{M>{W        ",
 "{M<{Cworn around arms{M>{W        ",
"{M<{Cworn on arms{M>{W            ",
"{M<{Cworn around wrist{M>{W       ",
"{M<{Cworn around wrist{M>{W       ",
"{M<{Cworn on hands{M>{W           ",
"{M<{Cworn on finger{M>{W          ",
"{M<{Cworn on finger{M>{W          ",
"{M<{Cwedding ring{M>{W            ",
"{M<{Cworn around body{M>{W        ",
"{M<{Cworn about body{M>{W         ",
"{M<{Cworn about waist{M>{W        ",
"{M<{Cworn around legs{M>{W        ",
"{M<{Cworn on legs{M>{W            ",
"{M<{Cworn on feet{M>{W            ",
"{M<{Cworn as shield{M>{W          ",
"{M<{Cwielded{M>{W                 ",
"{M<{Csecondary weapon{M>{W        ",
"{M<{Cheld{M>{W                    ",
"{M<{Cfloating nearby{M>{W         ",
"{W<{Mlodged in a leg{W>           ",
"{W<{Mlodged in an arm{W>          ",
"{W<{Mlodged in a rib{W>           ",
};
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if ( IS_EVIL(victim)
    &&   IS_AFFECTED(ch, AFF_DETECT_EVIL)     ) strcat( buf, "{M(Red Aura){x "   );
    if ( IS_GOOD(victim)
    ...
    if ( !IS_NPC(victim) && IS_SET(victim->act, PLR_WAR) ) 
                                                strcat( buf, "{M(WAR){x " );    
    if (!IS_NPC(victim) && !victim->desc      ) strcat( buf, "{M({MLinkdead{M){x " );        
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	sprintf(buf,"{M:{Cworn as cloak{M:{W     %s\n\r",obj->short_descr);
	send_to_char(buf,ch);
	return;
    }
    ...
    for ( iWear = 0; iWear < MAX_WEAR; iWear++ )
    {
		...
		&&   can_see_obj( ch, obj ) && obj->item_type == ITEM_BELT )
		{
			...
			printf_to_char( ch, "{M:{Cstrapped to %s{M:{W\n\r", obj->short_descr );
...
void do_score( CHAR_DATA *ch, char *argument )
{
	...
	printf_to_char(ch, "{C-------------------------------------------------------------------\n\r");
	printf_to_char(ch, "{CName{M:{W      %-15s   {CLevel{M:  {W%-12d   {CAge     {M:  {W%d\n\r", ch->name, ch->level, get_age(ch));
	printf_to_char(ch, "{CRace{M:{W      %-15s   {CSex  {M:  {W%-12s   {CSecurity{M:  {W%d\n\r", race_table[ch->race].name, ch->sex == 0 ? "sexless" : ch->sex == 1 ? "male" : "female", ch->pcdata->security);
	printf_to_char(ch, "{CClan{M:{W      %-15s   {CRank {M:  {W%-12s   {CHours   {M:  {W%d\n\r", is_clan(ch) ? player_clan(ch) : "None", is_clan(ch) ? player_rank(ch) : "0", ( ch->played + (int) (current_time - ch->logon) ) / 3600);
	printf_to_char(ch, "{C-------------------------------------------------------------------\n\r");
    printf_to_char(ch, "{CClass{M:{W %-8s\n\r", class_long(ch));
	printf_to_char(ch, "{C-------------------------------------------------------------------\n\r");
	if (!IS_SET(ch->comm2, COMM_AUTOLEVEL))
		printf_to_char(ch, "{CYou have {W%ld{C exp saved right now. You need {W%d{C exp for one level.\n\r", ch->pcdata->current_exp, exp_per_level( ch, ch->pcdata->points));
	else
		printf_to_char(ch, "You have {W%d{C experience and need {W%d{C til next level!!!\n\r", ch->exp, ((ch->level + 1) * exp_per_level(ch,ch->pcdata->points) - ch->exp) > 0 ? ((ch->level + 1) * exp_per_level(ch,ch->pcdata->points) - ch->exp) : 0);
	printf_to_char(ch, "{C-------------------------------------------------------------------\n\r");
	printf_to_char(ch, "{CStr {M:{W %d {M[{W%d{M]  {CHitPt       {M:{W %-5ld {Cof {W%-5ld  {CMob Kills    {M: {W%-7d{x\n\r", ch->perm_stat[STAT_STR], get_curr_stat(ch,STAT_STR), ch->hit,  ch->max_hit, ch->pcdata->kills_mob);
	printf_to_char(ch, "{CInt {M:{W %d {M[{W%d{M]  {CMana        {M:{W %-5ld {Cof {W%-5ld  {CMob Deaths   {M: {W%-7d{x\n\r", ch->perm_stat[STAT_INT], get_curr_stat(ch,STAT_INT), ch->mana,  ch->max_mana, ch->pcdata->deaths_mob);
	printf_to_char(ch, "{CWis {M:{W %d {M[{W%d{M]  {CMoves       {M:{W %-5ld {Cof {W%-5ld  {CPlayer Kills {M: {W%-7d{x\n\r", ch->perm_stat[STAT_WIS], get_curr_stat(ch,STAT_WIS), ch->move,  ch->max_move, ch->pcdata->kills_pc);
	printf_to_char(ch, "{CDex {M:{W %d {M[{W%d{M]  {CPracs       {M:{W %-4d            {CPlayer Deaths{M: {W%-7d{x\n\r", ch->perm_stat[STAT_DEX], get_curr_stat(ch,STAT_DEX), ch->practice, ch->pcdata->deaths_pc);
	printf_to_char(ch, "{CCon {M:{W %d {M[{W%d{M]  {CTrain       {M:{W %-4d            {CQuest Points {M: {W%-7d{x\n\r", ch->perm_stat[STAT_CON], get_curr_stat(ch,STAT_CON), ch->train, ch->pcdata->questpoints);
    	printf_to_char(ch, "{CHitR{M:{W %3d      {CCast Level  {M: {W%-3d             {CTrivia Points{M: {W%-7d{x\n\r", GET_HITROLL(ch), ch->cast_level, ch->pcdata->trivia);
	printf_to_char(ch, "{CDamR{M:{W %3d      {CCast Ability{M: {W%-3d             {CTotal Trivia{M : {W%-7d{x\n\r", GET_DAMROLL(ch), ch->cast_ability, ch->pcdata->totaltpoints);
	printf_to_char(ch, "{C-------------------------------------------------------------------{x\n\r");
	if(HAS_RELIGION(ch) )
	{    printf_to_char(ch, "{CYou're a %s in %s", ch->pcdata->religion->rank[prime_class(ch)][ch->pcdata->rank], ch->pcdata->religion->name );
	     if(IN_FACTION(ch) )
	     	printf_to_char(ch, ", which is faction of %s", ch->pcdata->religion->main->name );
	     send_to_char(".\n\r{x", ch );
	     printf_to_char(ch, "{C-------------------------------------------------------------------{x\n\r");
	}
	printf_to_char(ch, "{CGold  {M: {W%-7ld {CBank Gold  {M: {W%-5ld {CItems {M: {W%-7d {Cof {W%-7d\n\r", ch->gold, ch->pcdata->gold_bank,ch->carry_number, can_carry_n(ch));
	printf_to_char(ch, "{CSilver{M: {W%-7ld {CBank Silver{M: {W%-5ld {CWeight{M: {W%-7ld {Cof {W%-7d.\n\r", ch->silver, ch->pcdata->silver_bank, get_carry_weight(ch) / 10, can_carry_w(ch) /10);
	printf_to_char(ch, "{C-------------------------------------------------------------------{x\n\r");
	printf_to_char(ch, "{CWimpy set to {W%d{C hit points.{x\n\r", ch->wimpy );
	if (!IS_NPC(ch) && ch->pcdata->condition[COND_DRUNK]   > 10 )
		send_to_char( "{CYou are {Wdrunk{x.\n\r",   ch );
	if ( !IS_NPC(ch) && ch->pcdata->condition[COND_THIRST] ==  0 )
		send_to_char( "{CYou are {Wthirsty{x.\n\r", ch );
	if ( !IS_NPC(ch) && ch->pcdata->condition[COND_HUNGER]   ==  0 )
		send_to_char( "{CYou are {Whungry{x.\n\r",  ch );
    ...
    if ( percent >= 90 )
	send_to_char( "{CYou are well {Wrested{x.\n\r", ch );
    else if ( percent >= 75 )
	send_to_char( "{CYou feel {Wrested{x.\n\r", ch );
    else if ( percent >= 50 )
	send_to_char( "{CYou are a bit {Wtired{x.\n\r", ch );
    else if ( percent >= 25 )
	send_to_char( "{CYou feel {Wfatigued{x.\n\r", ch );
    else if ( percent >= 7 )
	send_to_char( "{CYou are {Wtired{x.\n\r", ch );
    else
	send_to_char( "{CYou are {Wasleep {Con your feet{x!\n\r", ch );

    switch ( ch->position )
	{	case POS_DEAD:
			send_to_char( "{CYou are {MD{ME{MA{MD{C!!{x\n\r",         ch );
			break;
		case POS_MORTAL:
			send_to_char( "{CYou are {Wmortally wounded{x\n\r",      ch );
			break;
		case POS_INCAP:
			send_to_char( "{CYou are {Wincapacitated{x\n\r", ch );
			break;
		case POS_STUNNED:
			send_to_char( "{CYou are {Wstunned{x\n\r",               ch );
			break;
		case POS_SLEEPING:
			send_to_char( "{CYou are {Wsleeping{x\n\r",              ch );
			break;
		case POS_RESTING:
			send_to_char( "{CYou are {Wresting{x\n\r",               ch );
			break;
		case POS_SITTING:
			send_to_char( "{CYou are {Wsitting{x\n\r",               ch );
			break;
        case POS_STANDING:
  			if(MOUNTED(ch))
			{	sprintf( buf, "{CYou are {Wriding on %s{x\n\r", MOUNTED(ch)->short_descr );
  				send_to_char( buf, ch);
			}
			else
				send_to_char( "{CYou are {Wstanding{x.\n\r",          ch );
	        break;

		case POS_FIGHTING:
			send_to_char( "{CYou are {Wfighting{x.\n\r",          ch );
			break;
	}
          
        if ( IS_MXP( ch ) )
        {
            printf_to_char(ch,"{CYou are in a " );
            ...
            send_to_char( buf, ch );
            printf_to_char(ch,"{Cstance{x\n\r" );
        }
        else
	printf_to_char(ch,"{CYou are in a {W%s{C stance{x\n\r", stance_table[ch->stance].name);

	if(RIDDEN(ch))
		printf_to_char(ch, "{CYou are ridden by {W%s{x\n\r", IS_NPC(RIDDEN(ch)) ? RIDDEN(ch)->short_descr : RIDDEN(ch)->name);

	if (ch->level >= 25)
		printf_to_char(ch, "{CArmor{M:\n\r");
   	for (i = 0; i < 4; i++)
	{	char * temp;
		...
		if   (GET_AC(ch,i) >=  101 )
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Chopelessly vulnerable{x\n\r",temp, GET_AC(ch,i) );
		else if (GET_AC(ch,i) >= 70)
			printf_to_char(ch,"{C\t%-8s{M:   [{W%d{M] -  {Cdefenseless{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= 30)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Cbarely protected{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= 10)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Cslightly armored{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= -10)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Csomewhat armored{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= -30)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Carmored{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= -50)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Cwell-armored{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= -200)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Cvery well-armored{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= -300)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Cheavily armored{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= -400)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Csuperbly armored{x\n\r",temp, GET_AC(ch,i));
		else if (GET_AC(ch,i) >= -500)
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Calmost invulnerable{x\n\r",temp, GET_AC(ch,i));
		else
			printf_to_char(ch,"\t{C%-8s{M:   [{W%d{M] -  {Cdivinely armored{x\n\r",temp, GET_AC(ch,i));

	}	

	printf_to_char(ch, "{C-------------------------------------------------------------------{x\n\r");
    if ( IS_IMMORTAL(ch))
    {	send_to_char("{CHoly Light{M:{W ",ch);
		...
		if (ch->invis_level)
  			printf_to_char(ch, "{C  Invisible{M: {W%d{x",ch->invis_level);
     	if (ch->incog_level)
  			printf_to_char(ch,"{C  Incognito{M: {W%d{x",ch->incog_level);
        
		send_to_char("\n\r",ch);
	}

		printf_to_char(ch, "{CAlignment{M: [{W%d{M] -{C  ", ch->alignment );
	...
	send_to_char( "{CMXP Enabled: [", ch );
        if(IS_MXP(ch))
        {
                send_to_char( "{MX{C]{x", ch );
        }
        else
        {
                send_to_char( " {C]{x", ch );
        ...
        send_to_char( "  {CMSP Enabled: [", ch );
        if (IS_MSP( ch ))
            send_to_char( "{MX{C]{x", ch );
        else 
            send_to_char( " {C]{x", ch );
        send_to_char( " {CMCCP Enabled: {C[{x", ch );
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    if(last_crash_time > (int) current_time || last_crash_time < first_boot_time )
        sprintf(crs_tme, "Eldoria hasn't crashed since Original Boot" );
    else
    {   
	...
	printf_to_char(ch, "{CEldoria's Time{M:            {W%s\n\r", sst );
	printf_to_char(ch, "{CEldoria's Day{M:             {W%s\n\r", ssd );
	printf_to_char(ch, "{CEldoria's Month{M:          {W %s\n\r", ssm );
	printf_to_char(ch, "{CEldoria's Last Crash{M:     {W %s\n\r", crs_tme );
	printf_to_char(ch, "{CEldoria's Original Boot{M:{W   %s\n\r", str_tme );
	printf_to_char(ch, "{CSystem time{M:{W                   %s{x\n\r", sys_tme );
    return;
}
...
void do_whois (CHAR_DATA *ch, char *argument)
{
	...
	if (!str_prefix(arg,wch->name))
	{
	    ...
	    /* a little formatting */
	    sprintf(buf, "\n\r{C[{B%3d {M%6s {M%s{C]{x %s%s%s%s%s%s%s%s%s%s%s%s\n\r",
		wch->level,
		...
		klass,
	     ...
	     IS_SET(wch->comm, COMM_AFK) ? "{W[{YAFK{W]{X " : "",
             ...
             IS_SET(wch->act, PLR_WAR) ? "{W({MWAR{W){X " : "",
             IS_SET(wch->act,PLR_KILLER) ? "{M<KILLER>{X " : "",
             IS_SET(wch->act,PLR_THIEF) ? "{M<{CTHIEF{>R{X " : "",
	    ...
	    sprintf(buf,"{c|                                                     |{x\n\r" );
            add_buf( output, buf );
            sprintf(buf,"{c| {CPkill Wins :{M %-10d {CPkill Losses :{M %-12d {c|{x\n\r",
...
void do_who( CHAR_DATA *ch, char *argument )
{
  ...
  for( level = MAX_LEVEL ; level > 0 ; level--)
  {
	...
	default: break;
       ...
       if (wch->level >= LEVEL_IMMORTAL && wch->pcdata->immtitle != NULL)
       {
          if (strlen_color(wch->pcdata->immtitle) == 14)
           sprintf(buf3, "{M[{W%s{M]{x", wch->pcdata->immtitle);
          ...
          else
          {
            ...
            sprintf(buf3, "{M[{W%*c%s%*c{M]{x", half, ' ', wch->pcdata->immtitle, sechalf,
              ' ');
          }
       }
       else
          sprintf( buf3, "{M[{W%3d {w%6s {C%s{M]{x",
             wch->level,
    ...
    send_to_char("{C\\_   _____/|  |    __| _/___________|__|____  \n\r",ch);
    send_to_char("{C |    __)_ |  |   / __ |/  _ \\_  __ \\  \\__  \\  \n\r",ch);
    send_to_char("{C |        \\|  |__/ /_/ (  <_> )  | \\/  |/ __ \\_ \n\r",ch);
    send_to_char("{C/_______  /|____/\\____ |\\____/|__|  |__(____  / \n\r",ch);
    send_to_char("{C        \\/            \\/                    \\/  \n\r",ch);
    send_to_char("\n\r                     MUD                          \n\r{x",ch);
if(immortal_online() > 0 )
send_to_char("{CImmortals Online{M: {X\n\r",ch);

    if ( double_exp )
    {
     sprintf( buf2,"\n\r{CDouble exp is on for {W%d {Cticks!{x", global_exp );
     add_buf(output,buf2);
    }  
    sprintf( buf2, "\n\r{CPlayers found{M: {W%d      {CMost on today{M    : {W%d{C\n\rMax on ever  {M: {W%d     {CTotal connections{M:{W %d{x\n\r", nMatch,
max_on, max, count_connections );
    add_buf(output,buf2);
    if (!ON_GQUEST(ch) && gquest_info.minlevel < ch->level &&
	gquest_info.maxlevel > ch->level)
	add_buf(output, "{CThere is a Global Quest running you can join.{x\n\r");
    page_to_char( buf_string(output), ch );
    free_buf(output);
    return;
}
...
void do_equipment( CHAR_DATA *ch, char *argument )
{
    ...
    for ( iWear = 0; iWear < MAX_WEAR; iWear++ )
    {
		...
		if ( found )
			send_to_char( "\n\r", ch );
		found = TRUE;
		printf_to_char( ch, "{M:{Cstrapped to %s{M:{W{x\n\r", obj->short_descr );
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	for ( sn = 0; sn < MAX_SKILL; sn++ )
	{
		...
		continue;
            sprintf(buf2,"{C%s{M:{W",capitalize(skill_table[sn].name ));

		     if(ch->pcdata->learned[sn] == 1   ) sprintf(buf3, "{w%3d{C%%", ch->pcdata->learned[sn]);
		else if(ch->pcdata->learned[sn] <= 75  ) sprintf(buf3, "{W%3d{C%%", ch->pcdata->learned[sn]);
		   else                                  sprintf(buf3, "{c%3d{C%%", ch->pcdata->learned[sn]);
...
void do_finger( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level > LEVEL_IMMORTAL )
    {
        ...
        sprintf(buf,"{cName  : {C%s{x\n\r"
                ...
                "{mPkills: {M%-10d  {mPDeaths: {B%d{x\n\r"
                "{mMkills: {B%-10d  {mMDeaths: {M%d{x\n\r"
                "{mHdam  : {M%-10d  {gClan   : {G%s{x\n\r\n\r"  
                "Last {Clogged off{x on %s\n\r",   
                victim->name,
                victim->level,
                victim->trust,
...
void do_mobdeaths(CHAR_DATA * ch, char *argument)
{
    ...
    add_buf(buffer, "{M                 Eldorias Most Popular Mobs                       {x\n\r");
...
void do_mobkills(CHAR_DATA * ch, char *argument)
{
    ...
    add_buf(buffer, "{M                 Eldorias Most Dangerous Monsters                 {x\n\r");
    add_buf(buffer, "{GNum  Mob Name                  Level  Area Name            Deaths{x\n\r");
    add_buf(buffer, "---  ------------------------- -----  -------------------- ------{x\n\r");
  ...
  if (ch->level < MAX_LEVEL)	/* Stuff trusted characters! */
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH, "\n\r{CEldorias{x current version is : %s\n\r", version);
      send_to_char (buf, ch);
      return;
    }
      ...
      else
	{
	  if (write_version (argument))
	    {
	      sprintf (buf, "{CEldorias{x current version changed to %s\n\r", argument);
	      send_to_char (buf, ch);
	      sprintf (buf, "%s has changed {CEldorias{x current version number to %s",
		       ch->name, argument);
	      log_string (buf);
	    }
	  else
...
void do_affects2 (CHAR_DATA *ch, char *argument)
{
         ...
         IS_AFFECTED ( victim, race_table[victim->race].aff ) )
    {
        ...
        while ( buf2[0] )
        {
            snprintf ( buf, MAX_STRING_LENGTH, "Spell: %-19s", buf2 );
            add_buf ( buffer, buf );
            add_buf ( buffer, "\n\r" );
            buf4 = one_argument ( buf4, buf2 );
        }
        ...
        for ( iWear = 0; iWear < MAX_WEAR; iWear++ )
        {
            if ( ( obj = get_eq_char ( victim, iWear ) ) != NULL )
            {
                for ( paf = obj->affected; paf != NULL; paf = paf->next )
                {
                    ...
                    while ( buf2[0] )
                    {
                        snprintf ( buf, MAX_STRING_LENGTH, "Spell: %-19s: %s", buf2,
                                  obj->short_descr );
                        add_buf ( buffer, buf);
                        add_buf ( buffer, "\n\r" );
                        buf4 = one_argument ( buf4, buf2 );
...
void do_suppress(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg[0]=='\0')
  {
     send_to_char("{CS{cuppression settings{C:\n\r",ch);
     printf_to_char(ch,"{CW{ceapon flags{C:   %s  {CF{clag effects{C: %s  {CD{codge/etc.{C: %s{x\n\r",
     SUP_OO(ch,SUPPRESS_FLAGS), SUP_OO(ch,SUPPRESS_FLAG_EFFECTS), SUP_OO(ch,SUPPRESS_DODGE));
  
    printf_to_char(ch,"{CO{cbject effects{C: %s{x\n\r", SUP_OO(ch,SUPPRESS_BURN));
  ...
  send_to_char("{cSyntax{C: {xsuppress {cweapon{C/{cflags{C/{cdodge{C/{cobject{C/{cshields{x\n\r",ch);
  return;
