...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
        /* It says Eldoria may accept it later ... =) */
        if (number_percent() == 1)
        {
            act("$n offers $mself to Someone, who graciously accepts!", ch,NULL,NULL,TO_ROOM);
            act("The Gods appreciate your offer and accepts it!",ch,NULL,NULL,TO_CHAR);
            ch->mana = 1;
            ch->move = 1;
            ch->hit  = 1;
            return;
	...
	act( "$n offers $mself to Someone, who graciously declines.",ch, NULL, NULL, TO_ROOM );
        act( "The Gods appreciate your offer and may accept it later.\n\r", ch, NULL, NULL, TO_CHAR );
	return;
    }
 ...
 /*            if(!IS_NPC(ch) && ch->pcdata->religion != NULL )
	     {
	        ...
	        continue;
             ...
             printf_to_char( ch, "{CEldoria gives you {Y%d{c silver for your sacrifice of {w%s{c.{x\n\r", silver, obj->short_descr );
             act( "$n sacrifices $p to Eldoria.", ch, obj, NULL, TO_ROOM );
             ch->silver += silver;
             extract_obj( obj );
	...
	if (obj->contains)
        {
           act( "Eldoria wouldn't like that.\n\r", ch, NULL, NULL, TO_CHAR );
	   return;
        }
    }
    ...
    if (silver == 1)
        act( "Eldoria gives you one silver coin for your sacrifice.\n\r", ch,NULL, NULL, TO_CHAR );
    else
    {
	sprintf(buf,"Eldoria gives you %d silver coins for your sacrifice.\n\r",silver);
	send_to_char(buf,ch);
    }
