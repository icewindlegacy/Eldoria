...
void do_gain(CHAR_DATA *ch, char *argument)
{
	...
	for (gn = 0; gn < MAX_GROUP; gn++)
	{
	    ...
	    &&  group_rating(ch, gn) > 0)
	    {
		sprintf(buf,"%-18s %-5d",
		    group_table[gn].name,group_rating(ch, gn));
		send_to_char(buf,ch);
		if (++col % 3 == 0)
		    send_to_char("\n\r",ch);
	    }
	...
	col = 0;
        ...
        sprintf(buf, "%-15s %-5s %-5s %-15s %-5s %-5s %-15s %-5s %-5s \n\r",
                     "skill", "cost", "level", "skill", "cost", "level", "skill", "cost", "level");
        send_to_char(buf,ch);
	    ...
	    &&  skill_table[sn].spell_fun == spell_null)
            {
                sprintf(buf,"%-15s %-5d %-5d ",
                    skill_table[sn].name,skill_rating(ch, sn), skill_level(ch,sn));
                send_to_char(buf,ch);
                if (++col % 3 == 0)
                    send_to_char("\n\r",ch);
            }
    ...
    if (!str_prefix(arg,"revert"))
    {
        ...
        act("$N helps you revert your training session into practices.",
                ch,NULL,trainer,TO_CHAR);
        ch->practice += 10;
        ch->train -= 1 ;
        return;
    ...
    if (!str_prefix(arg, "convertall"))
    {
        if ( ch->practice < 20 )
        {
            send_to_char( "You don't have enough practices just use convert.\n\r", ch );
            return;
        }

        ch->train += ( ch->practice / 10 );
        ch->practice = 0;
        printf_to_char( ch, "%s takes all of your practices and turns them into trains!\n\r", trainer->name );
        return;
    }

    if (!str_prefix(arg, "revertall"))
    {
        if ( ch->train < 2 )
        {
            send_to_char( "You don't have enough trains just use revert!\n\r", ch );
            return;
        }

        ch->practice += ( ch->train * 10 );
        ch->train = 0;
        printf_to_char( ch, "%s takes all your trains and turns them into practices!\n\r", trainer->name );
        return;
    }
...
void do_spells(CHAR_DATA *ch, char *argument)
{
	...
	&&  ch->pcdata->learned[sn] > 0)
	    ...
	    if (ch->level < level)
	    	sprintf(buf,"{B%-18s{c n/a{x      ", skill_table[sn].name);
	    else
	    {
		mana = UMAX(skill_table[sn].min_mana,
		    100/(2 + ch->level - level));
	        sprintf(buf,"{W%-18s  {D%3d mana{x  ",skill_table[sn].name,mana);
	    }
 
	    if (spell_list[level][0] == '\0')
          	sprintf(spell_list[level],"\n\r{DLevel %2d{r: %s{x",level,buf);
	    else /* append */
	    {
    ...
    if (!found)
    {
      	send_to_char("{DNo {Wspells{D found.{x\n\r",ch);
      	return;
    }
...
void do_skills(CHAR_DATA *ch, char *argument)
{
	...
	&&  ch->pcdata->learned[sn] > 0)
	    ...
	    level = skill_level(ch, sn);
	    if (ch->level < level)
	    	sprintf(buf,"{W%-18s {Dn/a      {x", skill_table[sn].name);
	    else
	    	sprintf(buf,"{W%-18s {D%3d%%     {x",skill_table[sn].name,
		    ch->pcdata->learned[sn]);
 
	    if (skill_list[level][0] == '\0')
          	sprintf(skill_list[level],"\n\r{DLevel %2d{r:{W %s",level,buf);
	    else /* append */
	    {
...
bool parse_gen_groups(CHAR_DATA *ch,char *argument)
{
	...
	if (gn != -1)
	{
	    ...
	    /* Close security hole */
	    if (ch->gen_data->points_chosen + group_rating(ch, gn)
		> 300)
	    {
		send_to_char(
	...
	if (sn != -1)
	{
	    ...
	    /* Close security hole */
	    if (ch->gen_data->points_chosen + skill_rating(ch, sn)
		> 300)
	    {
		send_to_char(
		    "You cannot take more than 300 creation points.\n\r", ch);
