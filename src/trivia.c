...
void close_trivia( void )
{
	...
	sprintf( buf,"{wThe {WEldoria {wOfficial {gT{Gr{Wiv{Gi{ga{w Contest is now Closed!{x\n\r");
	talk_trivia( buf );
...
void do_trivia( CHAR_DATA * ch, char *argument )
{
	...
	if ( !str_prefix( arg1, "open" ) )
	{
		...
		tLowRange = 0;
		tHighRange = total -1;
		sprintf( buf,"{wThe {WEldoria {wOfficial {gT{Gr{Wiv{Gi{ga{w Contest is now Open!{x\n\r");
		talk_trivia( buf );
                trivia->running = TRUE;
		trivia->question = NULL;
		trivia->answer = NULL;
